...
// <!-- timezone related code
		if (Configure::read('site.timezone_offset')) {
			foreach($this->data[$this->alias] as $field => $value) {
				if ((preg_match('/_date$/', $field) || preg_match('/_time$/', $field) || $field == 'listing_appears_on_site') && $field != 'is_show_month_date') {
					$hour_var = split(':', $value);
					if (!empty($hour_var[1])) {
						$this->data[$this->alias][$field] = gmdate('Y-m-d H:i:s', strtotime($this->data[$this->alias][$field]));
					} else {
						$this->data[$this->alias][$field] = gmdate('Y-m-d', strtotime($this->data[$this->alias][$field] . " " . gmdate('H:i:s', strtotime("now"))));
					}
				}
			}
		}
