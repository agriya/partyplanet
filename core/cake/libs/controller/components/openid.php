...
class OpenidComponent extends Object {
    ...
    public function authenticate($openidUrl, $returnTo, $realm, $dataFields = array()) {
        ...
        if ($authRequest->shouldSendRedirect()) {
            ...
            $this->showFormWithAutoSubmit($authRequest, $returnTo, $realm, $openidUrl);
        }
    }
    ...
    private function showFormWithAutoSubmit($request, $returnTo, $realm, $openidUrl) {
        ...
        $formHtml = $request->formMarkup($realm, $returnTo, false , array('id' => $formId), $domain);

        if (Auth_OpenID::isFailure($formHtml)) {
            throw new Exception('Could not redirect to server: '.$formHtml->message);
        }

        echo '<html><head><title>' . Configure::read('site.name') . ' |  Login - ' . ucfirst($domain) . '</title><link rel="stylesheet" type="text/css" href="'.$gobal_pth.'css/redirection.cache.css" /></head>' . "<body id='authorize' onload='document.getElementById(\"".$formId."\").submit()'>".
             $formHtml.'</body></html>';
        exit;
    }
