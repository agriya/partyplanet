        ...
        if (isset($_FILES['Filedata']['name']) and !empty($_SESSION['flashupload_data'][$controller->name])) {
            ...
            if ($multiple) {
                ...
                if ($controller->{$modelClass}->save($this->data, false)) {
                    ...
                    $uploaded_data=Cache::read(session_id());
                    if (!empty($uploaded_data)) {
                    array_push($uploaded_data,$controller->{$modelClass}->getLastInsertId());
                    Cache::write(session_id(), $uploaded_data);
                    } else {
                    Cache::write(session_id(), array($controller->{$modelClass}->getLastInsertId()));
                     }
                }
            } else {
                $attachments = array();
