 ...
 */
	public function requestAction($url, $extra = array()) {
		...
		if (is_array($url)) {
			if (!empty($this->request->params['named'][Configure::read('site.prefix_parameter_key')])) {
				$url[Configure::read('site.prefix_parameter_key')] = $this->request->params['named'][Configure::read('site.prefix_parameter_key')];
			} else {
				$url[Configure::read('site.prefix_parameter_key')] = Cache::read('site.prefix_url_value');
			}
		}
...
// quick fix for permanent cache to check requested
			$request->requested = 1;
