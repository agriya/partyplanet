		...
		if (!empty($object)) {
			...
			if (!empty($object->validate)) {
				foreach ($object->validate as $validateField => $validateProperties) {
					...
					$validateRules[$validateField] = array_reverse($validateProperties);
				}
			}
			$defaults = array('fields' => array(), 'key' => 'id', 'validates' => array(), 'validateRules' => array());
			$key = $object->primaryKey;
			$this->fieldset[$model] = array_merge($defaults, compact('fields', 'key', 'validates', 'validateRules'));
		}
...
// <--
		...
		if (!empty($divOptions) and !empty($this->fieldset[$modelKey]['validateRules'][$fieldKey])) {
			$divOptions = $this->addClass($divOptions, 'validation:' . str_replace('"', '\'', json_encode($this->fieldset[$modelKey]['validateRules'][$fieldKey])));
		}
 ...
 */
	public function minute($fieldName, $attributes = array()) {
		...
		if (is_string($attributes['value']) && strlen($attributes['value']) > 2) {
