 ...
 */
	...
	function router_url($url, $named = null)
    {
        $user_preferred = (!empty($_COOKIE['CakeCookie']['slug'])) ? $_COOKIE['CakeCookie']['slug'] : '';
        if ($tmp_url = (!empty($url[Configure::read('site.prefix_parameter_key')]) ? $url[Configure::read('site.prefix_parameter_key')] : (!empty($named[Configure::read('site.prefix_parameter_key')]) ? $named[Configure::read('site.prefix_parameter_key')] : $user_preferred))) {
            if ($tmp_url == '/') $tmp_url = $user_preferred;
            if (is_array($url)) {
                $url[Configure::read('site.prefix_parameter_key')] = htmlentities($tmp_url, ENT_QUOTES);
            } else if ($url == '/') {
                $url = array(
                    Configure::read('site.prefix_parameter_key') => htmlentities($tmp_url, ENT_QUOTES)
                );
            }
        }
        return $url;
