...
<div class="pages">
 ...
 <div class="message notice">
    ...
    <p>This is just a diagnostic setup file that set as home page in config/routes.php. Change the route before you start. Once you change the route, you can still access this page by <?php echo $this->Html->link('pages/install', array('controller' => 'pages', 'action' => 'display', 'install'));?></p>
        ...
        if ($readdir != '.' && $readdir != '..' && $readdir != '.svn') {
            ...
            if (is_file($path) && (strpos($path, 'install.ctp') === false) && ((strpos($path, '.php') !== false) || (strpos($path, '.ctp') !== false))) {
                ...
                if ((strpos($path, 'controllers') !== false) || (strpos($path, 'models') !== false)) {
                    ...
                    if (stripos($contents, '$this->request->params[\'requested\']') !== false) {
                        $err.= '<li><span class="error">Use of <code>$this->request->params[\'requested\']</code></span> Use recent draft on pagination instead</li>' . "\n";
                    }
