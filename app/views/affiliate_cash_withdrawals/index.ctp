...
<?php if(!empty($moneyTransferAccounts)) {?>
        <?php echo $this->element('../affiliate_cash_withdrawals/add', array('cache' => array('config' => 'site_element_cache_5_hours'))); ?>
<?php }
else
{ ?>
<div class="page-info">
	...
	<tr<?php echo $class;?>>
		...
		<?php 
			...
			elseif($userCashWithdrawal['AffiliateCashWithdrawalStatus']['id'] == ConstAffiliateCashWithdrawalStatus::Success):
				...
				echo "<p>".($this->Html->cCurrency($userCashWithdrawal['AffiliateCashWithdrawal']['amount'] - $userCashWithdrawal['AffiliateCashWithdrawal']['commission_amount'])).' = ['.$this->Html->cCurrency($userCashWithdrawal['AffiliateCashWithdrawal']['amount']).' - '.$this->Html->cCurrency($userCashWithdrawal['AffiliateCashWithdrawal']['commission_amount']).']'."</p>";
				endif;
			else:
