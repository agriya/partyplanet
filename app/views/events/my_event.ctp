			...
			<?php
				...
				<li class="clearfix <?php echo $class; ?>">
					<div class="grid_4 alpha">
						...
						<?php if($event['Event']['admin_suspend'] == 0 && $event['Venue']['admin_suspend'] == 0) { ?>
						<div class=" event-action-block">
					
							<?php
							if(strtotime($event['Event']['end_date']) > time()){
								if($event['Event']['is_bump_up'] == 0 || $event['Event']['is_featured'] == 0) {
                                     //  Tempporarily commented
									//echo $this->Html->link(__l('Promote'), array('controller'=> 'payments', 'action' => 'order', $event['Event']['slug'], 'event'), array('title'=>__l('Promote'), 'class' => 'promote', 'escape' => false));
								}
								echo $this->Html->link(__l('Edit'), array('controller'=> 'events', 'action' => 'edit', $event['Event']['id']), array('title'=>__l('Edit'), 'class' => 'edit', 'escape' => false));
							}
							?>
                        
							<?php  echo $this->Html->link(__l('Delete'), array('controller'=> 'events', 'action' => 'delete', $event['Event']['id']), array('class' => 'js-delete delete', 'title'=>__l('Delete'), 'escape' => false));?>
						</div>
						<?php } ?>
