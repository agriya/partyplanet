...
$facebook_settings_id = 40;
$twitter_settings_id = 41;
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<title>PartyPlanet - Diagnostic Tool</title>
<style type="text/css">
body{
	color:#4c4c4c;
	line-height:18px;
...
function _db_check($database = 'default',$facebook_settings_id,$twitter_settings_id)
{
    ...
    if ($link) {
        ...
        if ($db_selected) {
            ...
            $result = mysql_query("select * from settings where setting_category_parent_id in (1, 2, 15)", $link);
...
<h1>PartyPlanet Diagnostic Tool</h1>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=partyplanet-install#setting_up_files" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=partyplanet-install#setting_up_files</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=partyplanet-install#setting_up_database" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=partyplanet-install#setting_up_database</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=partyplanet-install#manage_settings" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=partyplanet-install#manage_settings</a>
	</span>
</p>
<?php
...
?>
	...
	<?php
		...
		{
			$setting['Mandatory'][11] = $setting['Mandatory'][1];
			$setting['Mandatory'][12] = $setting['Mandatory'][4];
            unset($setting['Mandatory'][1]);
			unset($setting['Mandatory'][4]);
		}	
		if($key == $twitter_settings_id) //To display twitter credentials in the last rows
		{
			$setting['Mandatory'][11] = $setting['Mandatory'][3];
			$setting['Mandatory'][12] = $setting['Mandatory'][4];
            unset($setting['Mandatory'][3]);
			unset($setting['Mandatory'][4]);
		}	
		if($key == 'iphone'){
			$temp = $setting['Mandatory'][0];
			$setting['Mandatory'][0] = $setting['Mandatory'][1];
			$setting['Mandatory'][1] = $temp;
		}
...
?>
						<tr>
							...
							<td class="<?php
                            echo $class; ?> "><?php 
							if ($mantry['name'] == 'subscription.iphone_apns_push_mail_enable') {
                                echo ($mantry['value']) ? 'On' : 'Off';
                            } elseif ($mantry['name'] == 'site.contact_email') {
...
?>
							...
							if ($mantry['name'] == 'subscription.iphone_certification_mode') {
                                if ($mantry['value'] == 'Live') {
                                    $class = 'green';
                                } else {
                                    $class = 'red';
                                }
                            ...
                            $settings_check_one_zero = array(
								...
								'subscription.iphone_apns_push_mail_enable'
                            );
